...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### START CODE HERE
        # Step 1: Retrieve and randomly truncate the document
        document = self.data[idx]
        max_len = int(self.block_size * 3 / 4)
        # Randomly selects a length between 4 and int(self.block_size * 3 / 4)
        truncated_len = random.randint(4, max_len)
        truncated_doc = document[:truncated_len]
        
        # Step 2: Randomly split the document into prefix, masked_content, and suffix
        mask_len = max(1, random.randint(1, len(truncated_doc) // 4))
        start = random.randint(0, len(truncated_doc) - mask_len)
        prefix = truncated_doc[:start]
        masked_content = truncated_doc[start:start + mask_len]
        suffix = truncated_doc[start + mask_len:]
        
        # Step 3:Rearrange and construct the masked_string
        masked_string = (
            prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content +
            self.MASK_CHAR + self.PAD_CHAR * (self.block_size - len(prefix + suffix + masked_content) - 3)
        )
        ...
        # Step 5: Convert x and y to tensor format
